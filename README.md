...
All values below are from the CSV/JSON exports in `results/` generated by the
notebook's latest reported run.
...
- Maximum absolute difference between analytic and finite-difference gradients: `1.63e-10`.
- Mean full-to-screening ROC-AUC decrement across the 12 evaluated models: `0.4025`.
...
- Under the recorded distribution shifts, `tau*` is 2.6% above the target oracle cost, while a source-tuned threshold is 365% above it.
...
The notebook resolves the project root automatically, creates `results/` and `figures/` when needed, and overwrites exported artifacts during a fresh run. The full run is CPU-only and takes approximately 40 minutes on the reference setup. The current run exports 21 result artefacts and 14 figures, with each figure saved as both PDF and PNG.
